...
class Simulator(val taskSupport: TaskSupport, val timeStats: TimeStatistics) {
  ...
  def updateBoundaries(boundaries: Boundaries, body: Body): Boundaries = {
    ...
    boundary.maxX = if(body.x > boundaries.maxX) body.x else boundaries.maxX
    boundary.minX = if(body.x < boundaries.minX) body.x else boundaries.minX
    boundary.minY = if(body.y < boundaries.minY) body.y else boundaries.minY
    boundary.maxY = if(body.y > boundaries.maxY) body.y else boundaries.maxY
    boundary
  }

  def mergeBoundaries(a: Boundaries, b: Boundaries): Boundaries = {
    val boundary = new Boundaries
    boundary.maxX = a.maxX max b.maxX
    boundary.minX = a.minX min b.minX
    boundary.maxY = a.maxY max b.maxY
    boundary.minY = a.minY min b.minY
    boundary    
  }
