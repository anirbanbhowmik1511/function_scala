...
//      val dnw = distance(b.x, b.y, nw.centerX, nw.centerY)
//      val dne = distance(b.x, b.y, ne.centerX, ne.centerY)
//      val dsw = distance(b.x, b.y, sw.centerX, sw.centerY)
//      val dse = distance(b.x, b.y, se.centerX, se.centerY)
//      val min = dnw min dne min dsw min dse
      Fork(if(b.x <= centerX && b.y <= centerY) nw.insert(b) else nw, if(b.x >= centerX && b.y <= centerY) ne.insert(b) else ne, if(b.x <= centerX && b.y >= centerY) sw.insert(b) else sw, if(b.x >= centerX && b.y >= centerY) se.insert(b) else se)
    }
  }
  ...
  extends Quad {
    ...
    def insert(b: Body): Quad = {
      if(size <= minimumSize) Leaf(centerX, centerY, size, b +: bodies)
        ...
        bodies.foldRight(f)((b, f) => f.insert(b))
      }
    }
  }
  ...
  class SectorMatrix(val boundaries: Boundaries, val sectorPrecision: Int) {
    ...
    def +=(b: Body): SectorMatrix = {
      val x = findPosition(b.x, boundaries.maxX, boundaries.minX, sectorPrecision)
      val y = findPosition(b.y, boundaries.maxY, boundaries.minY, sectorPrecision)
      matrix(y * sectorPrecision + x) += b
      this
    }
    
    def findPosition(v : Float, max : Float, min : Float, precision : Int) : Int = {
      if(v >= max) precision - 1
      else if(v <= min) 0
      else {
        val size = (max - min) / precision
        val res = math.floor((v - min) / size).toInt
        if(res == precision) res - 1 else res
      }
    }
    ...
    def combine(that: SectorMatrix): SectorMatrix = {
      this.matrix.zip(that.matrix).foreach(t => t._1.combine(t._2))
      this
    }
